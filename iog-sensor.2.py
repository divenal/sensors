...
def main():
    ...
    while True:
        ...
        if zappi.status <= 0 or zappi.mode != 3 :
            ...
            total = 0
            s = Sensors.IOG(int(now), 0, 0)
        elif (
              (zappi.status == 3 and seen == 0)     # charging now but have never seen any dispatches - only just plugged in ?
          or  (delta >= 600 and zappi.status == 3)  # every 10 mins while charging
          or  (delta >= 1200 and total > 0)         # every 20 mins while there are pending dispatches
          or  (delta >= 3600 and seen != 0)         # once an hour if there are no dispatches but we have seen some
          or  (delta >= 1800)                       # or every 30 mins if nothing planned (all finished ?)
            ):
